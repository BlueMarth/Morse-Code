...
#######################################
# def button_release(key): #what to do on button-release
#     time_taken = round(time.time() - t, 2)*1000 #rounding the long decimal float
#     print("The key",key," is pressed for",time_taken,'ms')
#     return False #stop detecting more key-releases

# def button_press(key): #what to do on button-press
#     return False #stop detectding more key-presses

# with keyboard.Listener(on_press = button_press) as press_listener: #setting code for listening key-press
#     press_listener.join()

# t = time.time() #reading time in sec

# with keyboard.Listener(on_release = button_release) as release_listener: #setting code for listening key-release
#     release_listener.join()
#######################################
